...
# -*- coding: utf-8 -*-
"""
Manual ScriptPubKey Example (P2WPKH) - manual signing version with optional nLockTime demo
...
Additionally, it allows you to set an absolute block-height nLockTime for learning purposes.
"""
...
def send_with_fixed_change(wallet, to_address, amount_satoshis, change_address, fee_satoshis=1000):
    ...
    try:
        ...
        # Optional: Set absolute nLockTime (block height) for this transaction.
        # NOTE: nLockTime is a property of the *whole transaction*.
        #       Until the specified height/time is reached, the tx is non-final
        #       (cannot enter mempool or be mined), not "UTXO exists but is locked".
        locktime_str = input("\nLock until block height (0 for no locktime, press Enter to skip): ").strip()
        if locktime_str:
            locktime_height = int(locktime_str)
            if locktime_height < 0:
                raise ValueError("Locktime height must be non-negative")

            # Set transaction-level locktime
            tx.locktime = locktime_height
            # Some versions also track locktime_int; guard with getattr
            if hasattr(tx, "locktime_int"):
                tx.locktime_int = locktime_height

            # For nLockTime to be enforced, at least one input must have sequence < 0xffffffff
            for inp in tx.inputs:
                # Keep sequence as an int; Transaction.serialize() / internals will handle conversion
                inp.sequence = 0xFFFFFFFE

            print(f"Set nLockTime to block height {locktime_height} and updated input sequence values.")
        ...
        # Show raw transaction so it can be pushed manually if needed
        try:
            raw_tx = tx.raw()
            raw_hex = raw_tx.hex()
        except Exception:
            raw_hex = "<could not serialize raw tx>"
...
def main():
    """Demo workflow demonstrating signet transaction, manual scriptPubKey generation, manual signing, and optional nLockTime."""
    print("=== Bitcoin Signet Transaction Demo (Manual Signing + nLockTime) ===\n")
